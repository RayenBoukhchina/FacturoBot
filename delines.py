...
def stretch_columns(img):
	...
	cv2.imwrite("output_delines/columnstretched.jpg",x)
	return x
...
def segment_columns(img,shape,contours):
	...
	cv2.imwrite("output_delines/blocks.jpg",img)
	return cont
...
if __name__ == "__main__":
	...
	save_dir = "output_delines/"

	# Créer le dossier output_delines si inexistant
	...
	if img is None:
		print(f"❌ Image '{image_path}' introuvable ou illisible.")
		exit()

	# Étape 1 : Suppression des lignes et extraction de contours
	contours, hierarchy, bw = ignore_lines(img, save_dir, file_name)

	# Étape 2 : Segmentation des colonnes et dessin des blocs détectés
	shape = bw.shape
	img_blocks = cv2.imread(image_path)  # on recharge l’image originale pour dessiner dessus
	_ = segment_columns(img_blocks, shape, {'contours': [cv2.boundingRect(c) for c in contours]})

	print("✅ Traitement terminé. Résultats enregistrés dans le dossier 'output_delines/'.")
